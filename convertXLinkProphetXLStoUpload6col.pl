...
my $SELF = 'convertXLinkProphetXLStoUpload6col.pl';
...
if(@ARGV == 0) {
	...
	printf STDERR "          Use to filter XLinkProphet output XLS file based on minimum probability to produce 6-column file [ pepA, proA, kposA, pepB, proB, kposB ] ready for upload to XLinkDB\n\n";
...
while(<FILE>) {
	...
	if($first) {
		...
		printf OUT "%s\t%s\t%d\t%s\t%s\t%d%s\n", $parsed[$headers{"peptide1"}], getUniprot($parsed[$headers{"protein1"}]), $kposA, $parsed[$headers{"peptide2"}], getUniprot($parsed[$headers{"protein2"}]), $kposB, $extra_cols; #, $parsed[$headers{"composite_id"}];
...
close(FILE);
close(OUT);
printf "%d cross-links in %s with %s %s or greater written to 6-column file %s ready for upload to XLinkDB\n", $tot, $file, $USE_PROBABILITY ? "probability" : "composite_probability", $filter_thresh, $outfile;
